...
def get_predictorname():
    list_meta_files = [
        os.path.join(
            ...
            params['path']) if f.endswith(".meta")]
...
def updateGenreResult(
    ...
    for i in range(len(genreresult['incremental_prediction'])):
        ...
        for j in range(len(combinedlable)):
            if j < genrelevel:
                value = genreresult['incremental_prediction'][i][genreresult['label'].index(
                    combinedlable[j])]
                others = others - value
                row.append(value)
    ...
    for i in range(len(style1RespObj['incremental_prediction'])):
        ...
        for j in range(len(combinedlable)):
            if j < genrelevel:
                ...
                value = style1RespObj['incremental_prediction'][i][style1RespObj['label'].index(
                    combinedlable[j])]
                others = others - value
                row.append(value)
            elif j < (len(combinedlable) - 1):
    ...
    for i in range(len(style2RespObj['incremental_prediction'])):
        ...
        for j in range(len(combinedlable)):
            if j < genrelevel:
                ...
                value = style2RespObj['incremental_prediction'][i][style2RespObj['label'].index(
                    combinedlable[j])]
                others = others - value
                row.append(value)
            else:
...
def updateMultilayerResult(resultList, genrelevel):
    ...
    for i, result in enumerate(resultList):
        ...
        buildLableWithIndex(
            combinedlable, result, min(
                genrelevel, len(
                    result['label'])), index_dict, i)
        ...
        for i in range(len(result['incremental_prediction'])):
            ...
            for j in range(len(combinedlable)):
                if j < start:
                    ...
                    value = result['incremental_prediction'][i][result['label'].index(
                        combinedlable[j])]
                    others = others - value
                    row.append(value)
                elif j < (len(combinedlable) - 1):
